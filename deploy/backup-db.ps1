# Copia de seguridad de la base de datos de producción de MeDeben.
#
...
param(
    ...
    [string]$Destino = "$env:USERPROFILE\Documents\medeben-backups",
...
Write-Output "[backup] pg_dump de 'medeben' en el servicio db..."
docker compose -f $compose exec -T db pg_dump -U medeben -Fc -f "/tmp/$fichero" medeben
...
$purgados = Get-ChildItem $Destino -Filter 'medeben-*.dump' |
