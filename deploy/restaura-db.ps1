# Restauración de un backup de MeDeben.
#
...
if ($SobreLaBaseDeProduccion) {
    ...
    docker compose -f $compose exec -T db pg_restore -U medeben --clean --if-exists -d medeben "/tmp/$nombre"
...
docker run -d --name $drill -e POSTGRES_PASSWORD=drill -e POSTGRES_DB=medeben postgres:16 | Out-Null
...
try {
    ...
    docker exec $drill pg_restore -U postgres --no-owner -d medeben "/tmp/$nombre"
    ...
    docker exec $drill psql -U postgres -d medeben -t -A -F ': ' -c "SELECT 'usuarios' AS tabla, count(*) FROM usuarios UNION ALL SELECT 'perfiles', count(*) FROM perfiles UNION ALL SELECT 'cuadrantes', count(*) FROM cuadrantes UNION ALL SELECT 'apuntes', count(*) FROM apuntes;"
