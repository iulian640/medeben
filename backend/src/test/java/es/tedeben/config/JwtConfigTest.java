...
class JwtConfigTest {
    ...
    void sinPerfilActivoConSecretoDeJugueteNoArranca() {
        contexto.withPropertyValues(
                        ...
                        "tedeben.seguridad.jwt.secreto=" + SECRETO_DE_JUGUETE)
                .run(ctx -> {
                    assertThat(ctx).hasFailed();
                    assertThat(ctx.getStartupFailure())
                            .rootCause()
                            .isInstanceOf(IllegalStateException.class)
                            .hasMessageContaining("TEDEBEN_SEGURIDAD_JWT_SECRETO");
                });
    }
    ...
    void sinPerfilConSecretoRealArranca() {
        contexto.withPropertyValues("tedeben.seguridad.jwt.secreto=" + SECRETO_REAL)
                .run(ctx -> assertThat(ctx).hasNotFailed());
    }
    ...
    void secretoCortoNoArranca() {
        contexto.withPropertyValues("tedeben.seguridad.jwt.secreto=demasiado-corto")
                .run(ctx -> {
                    assertThat(ctx).hasFailed();
                    assertThat(ctx.getStartupFailure())
                            .rootCause()
