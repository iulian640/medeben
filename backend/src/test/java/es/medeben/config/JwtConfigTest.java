...
class JwtConfigTest {
    ...
    void sinPerfilActivoConSecretoDeJugueteNoArranca() {
        contexto.withPropertyValues(
                        ...
                        "medeben.seguridad.jwt.secreto=" + SECRETO_DE_JUGUETE)
                .run(ctx -> {
                    assertThat(ctx).hasFailed();
                    assertThat(ctx.getStartupFailure())
                            .rootCause()
                            .isInstanceOf(IllegalStateException.class)
                            .hasMessageContaining("MEDEBEN_SEGURIDAD_JWT_SECRETO");
                });
    }
    ...
    void sinPerfilConSecretoRealArranca() {
        contexto.withPropertyValues("medeben.seguridad.jwt.secreto=" + SECRETO_REAL)
                .run(ctx -> assertThat(ctx).hasNotFailed());
    }
    ...
    void secretoCortoNoArranca() {
        contexto.withPropertyValues("medeben.seguridad.jwt.secreto=demasiado-corto")
                .run(ctx -> {
                    assertThat(ctx).hasFailed();
                    assertThat(ctx.getStartupFailure())
                            .rootCause()
