...
class CuentaServiceTest {
    ...
    void arranque() {
        ...
        informesAnuales = mock(InformeAnualService.class);
        servicio = new CuentaService(usuarios, passwordEncoder, informesAnuales);
    ...
    @Test
    @DisplayName("además de borrar, purga la caché de informes anuales (su PDF lleva email y salarios)")
    void purgaLaCacheDeInformes() {
        when(usuarios.findById(USUARIO_ID)).thenReturn(Optional.of(usuario));
        when(passwordEncoder.matches("laBuena123", "$2a$10$hash")).thenReturn(true);

        servicio.borraCuenta(USUARIO_ID, "laBuena123");

        verify(informesAnuales).invalida(USUARIO_ID);
    }
    ...
    void passwordIncorrectaNoBorra() {
        ...
        verify(informesAnuales, never()).invalida(any());
    }
