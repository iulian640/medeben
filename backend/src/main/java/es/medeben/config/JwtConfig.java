 ...
 * en producción SIEMPRE por variable de entorno (MEDEBEN_SEGURIDAD_JWT_SECRETO),
...
public class JwtConfig {
    ...
    @Bean
    public JwtEncoder jwtEncoder(@Value("${medeben.seguridad.jwt.secreto}") String secreto,
                                 Environment entorno) {
        return new NimbusJwtEncoder(new ImmutableSecret<>(clave(secreto, entorno)));
    }

    @Bean
    public JwtDecoder jwtDecoder(@Value("${medeben.seguridad.jwt.secreto}") String secreto,
                                 Environment entorno) {
        ...
        if (!perfilNoProductivo && SECRETOS_DE_JUGUETE.contains(secreto)) {
            throw new IllegalStateException(
                    "Secreto JWT de desarrollo detectado fuera de dev/local/test: configura "
                            + "MEDEBEN_SEGURIDAD_JWT_SECRETO con un secreto real (p. ej. openssl rand -base64 48)");
        }
