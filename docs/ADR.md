...
- **Día que NO CUADRA (issue #230, 2026-07-11):** cuando la lectura derivada se contradice — tramos que se pisan en el reloj del día, una entrada abierta dentro de un tramo cerrado (la fusión del turno partido reconstruido en desorden), o salidas huérfanas que ninguna lectura recoge — el día pasa al estado **NO_CUADRA**: sin tramos ni total (la lectura que había engaña), excluido de los agregados, y **visible** en la libreta (día y semana), en el resumen mensual (aviso con fechas, el primero de la lista) y en los informes PDF (etiqueta en el diario, fila en el resumen, agregado en el histórico anual). Principio de la issue: no se adivina la intención del usuario (eso pediría el id de tramo, decisión abierta arriba) — solo se deja de perder horas en silencio; ni un total plausible pero falso ni un cero mudo. Una huérfana queda "explicada" (no alarma) si algún tramo termina exactamente a su misma hora. Del NO_CUADRA se sale re-apuntando el turno en orden o, si la lectura fundida persiste, con la frontera de AUSENCIA y el día fichado de nuevo.
