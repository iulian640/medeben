...
5. **(v3) Geofencing** — fichaje automático por ubicación; opt-in explícito, RGPD serio, batería. El final del camino. **NOTA (2026-07-26): D39 no es esto.** D39 («Anotar dónde fichas») es una anotación puntual en primer plano, disparada por un toque, sin background y sin geofencing — este punto (D15.5) sigue aparcado a v3.
...
### D39 — Anotar dónde fichas: ubicación aproximada opcional en el fichaje (Iulian + Claude, 2026-07-26)
Analizado en profundidad por tres lentes independientes (probatoria, RGPD, MVP) y cerrado en `Documents/medeben-gps-analisis/` (`sintesis.md`, `veredictos.md`, `decisiones-implementacion.md` — este último es el contrato vinculante). Iulian ordenó implementar ya; el matiz de calendario es que la feature nace **apagada de fábrica, opt-in, invisible**: mergearla no cambia nada para quien no la activa, y no se sube ningún artefacto a Play mientras el lanzamiento siga bloqueado por la verificación de dispositivo físico.

- **Nombre y encuadre:** en ningún texto de UI, de Play o legal aparecen las palabras **"GPS"** ni **"geolocalización"**. La feature se llama **«Anotar dónde fichas»**. Un fichaje fuera de radio se rotula «en otra ubicación», nunca «incidencia».
- **Qué se captura y cuándo:** posición aproximada, `ACCESS_COARSE_LOCATION` únicamente (con `xmlns:tools` + `<uses-permission android:name="android.permission.ACCESS_FINE_LOCATION" tools:node="remove"/>`, porque el manifest merger del plugin `@capacitor/geolocation` añade `FINE` sola — hay que verificarlo leyendo el manifest **mergeado**, no el declarado). Un punto por fichaje, **solo** en fichajes `CONFIRMADO` al momento — nunca en manual, reconstruido, ausencia ni rectificación tardía.
- **Regla de servidor adicional (corrección crítica de los verificadores):** `origen == CONFIRMADO` no basta para distinguir "al momento" de "manual de hoy con hora tecleada". Se exige además `|apunte.hora − hora local de apunte.registradoEn| ≤ 3 min` (misma zona del `Clock` inyectado); si no coincide, 422.
- **El servidor calcula el veredicto sobre dato bruto:** coordenadas crudas en BD (recomputables por un perito), nunca un booleano que declare el cliente. Endpoint `POST /api/v1/fichajes/{apunteId}/ubicacion` — 201 con `UbicacionResponse`, 409 si ya existe, **403 explícito** si no hay consentimiento vigente (nunca un 204 mudo). El consentimiento vigente se exige como PRIMER filtro en todo el namespace `/ubicacion` y `/centro-trabajo`.
- **Fórmula del veredicto** (`d` = distancia Haversine al centro, `p` = precisión reportada, `r` = radio — **150 m fijo, lo pone el servidor**, se revisa con los datos de campo de la Fase 0):
  - `p > 200` → `NO_CONCLUYENTE` (un fix así no afirma nada).
  - `d + 2p ≤ r` → `DENTRO`.
  - `d − 2p > r` → `FUERA`.
  - resto → `NO_CONCLUYENTE`.
  - La constante se llama `FACTOR_SIGMA = 2`, documentada: `Location.getAccuracy()` en Android es un radio de confianza al 68 % (1 sigma), no una cota dura del error, así que se dobla para acercarse al 95 % antes de afirmar nada en un documento que puede acabar en un SMAC.
- **Retención:** hasta 15 meses desde cada fichaje, **salvo que el usuario declare una reclamación en curso** (flag propio que suspende la purga, con revisión anual). Purga automática programada, mismo patrón que `SesionesPurga`. El aviso previo a la purga (14 meses) queda **pendiente antes de activar la feature en producción real** con usuarios.
- **Base jurídica:** consentimiento **inequívoco y expreso (art. 6.1.a RGPD)**, recabado mediante **acción afirmativa y casilla no premarcada** — nunca «consentimiento explícito» a secas, que es el estándar reforzado del art. 9. La app funciona entera sin esto.
- **Art. 9 por inferencia (nunca negación absoluta):** la ubicación aproximada no constituye por sí misma categoría especial, pero **puede revelarla por inferencia** (un fichaje recurrente coincidiendo con un centro sanitario, una sede sindical, un lugar de culto). Por eso se trata con las mismas medidas operativas que el `motivo`: nunca en logs, nunca en respuestas ordinarias de API, retención propia y más corta.
- **Reglas de oro (si una se rompe, parar):**
  1. Nunca en el camino crítico de fichar — ni un `await` entre el toque del usuario y el `POST /api/v1/fichajes`.
  2. Solo en fichajes al momento: `CONFIRMADO` + ventana de 10 min + coherencia hora/sello ±3 min.
  3. El servidor decide el veredicto, sobre dato bruto recomputable.
  4. Las coordenadas no salen en el PDF que ve la empresa — solo distancia relativa, y solo en el anexo técnico que controla el titular.
  5. No se tocan: `Apunte`, `OrigenApunte`, `TipoApunte`, `EstadoDia`, `derivaEstado()`, `V4__apuntes.sql`, `ApunteRepository`, `FichajeController`.
- **Orden de merge obligado:** los textos legales y de Play (privacidad, disclaimers, data-safety, iarc, ficha) se mergean **antes o en el mismo momento** que el manifest con el permiso de ubicación. Nada se sube a ninguna pista de Play, ni siquiera interna, mientras la fila «Ubicación» de `docs/play/data-safety.md` no diga «Sí».
- **Checklist RAT/EIPD pendiente** (documentos internos, fuera del repo, en `Documents/medeben-legal-interno` — entradas a actualizar antes de activar la feature con usuarios reales):
  1. **Reevaluar la EIPD** con los criterios que sí aplican — Directrices WP248 + lista de tratamientos que requieren EIPD de la AEPD: datos de localización + interesados vulnerables por relación laboral. **Nunca citar el art. 35.3.c** (observación sistemática de una zona de acceso público — no es el supuesto de esta feature).
  2. Documentar el **riesgo de coacción del empresario** (que el consentimiento del art. 6.1.a deje de ser libre si un encargado exige activar la opción o entregar el anexo con coordenadas) y su mitigación: aviso explícito en la pantalla de activación y en el PDF citando el art. 90 LOPDGDD, y enlace a cómo reclamar ante la AEPD.
  3. Declarar que la **resolución de la posición aproximada la realiza el proveedor de servicios de ubicación del sistema operativo (Google Play Services)**, como responsable independiente de ese tratamiento y según su propia política — fuera del control de MeDeben. Debe figurar también en la política de privacidad.
  4. Procedimiento **DSAR** (arts. 15/20 con ubicación incluida): entrega siempre como **descarga reautenticada**, **nunca por email**.
- **NOTA junto a D15.5:** D39 **no implementa** D15.5. D15.5 es geofencing — fichaje **automático** por ubicación, con monitorización en segundo plano. D39 es una **anotación puntual, en primer plano, disparada por un toque explícito** del usuario sobre un fichaje que ya se guardó: sin `ACCESS_BACKGROUND_LOCATION`, sin `watchPosition`, sin Geofence API, sin servicio en primer plano, sin fichaje automático. D15.5 sigue aparcada a v3.
